...
def calculate_energy(image):
    """Tính toán bản đồ năng lượng của ảnh."""
...
def find_seam(energy):
    """Tìm đường seam có năng lượng thấp nhất."""
...
def draw_seam(image, seam):
    """Vẽ một đường seam lên ảnh."""
    img_with_seam = image.copy()
    
    # Vẽ đường seam bằng màu đỏ
    for i in range(len(seam)):
        img_with_seam[i, seam[i]] = [0, 0, 255]  # Màu đỏ
...
def seam_carve(image, new_width):
    """Thay đổi kích thước ảnh bằng seam carving."""
...
# Đọc ảnh
image = cv2.imread('d:/Downloads/cvb.jpg')

# Chuyển ảnh sang ảnh xám và tính bản đồ năng lượng
...
# Vẽ đường seam màu đỏ lên ảnh gốc
image_with_seam = draw_seam(image, seam)

# Thay đổi kích thước ảnh bằng seam carving
new_width = 300  # Kích thước chiều ngang mong muốn
resized_image = seam_carve(image.copy(), new_width)
...
# Hiển thị bản đồ năng lượng
plt.subplot(1, 4, 2)
plt.imshow(energy, cmap='gray')
plt.title('Energy Map')
plt.axis('off')

# Hiển thị ảnh gốc với đường seam màu đỏ
plt.subplot(1, 4, 3)
plt.imshow(cv2.cvtColor(image_with_seam, cv2.COLOR_BGR2RGB))
plt.title('Image with Lowest Energy Seam')
plt.axis('off')

# Hiển thị ảnh sau khi thay đổi kích thước
plt.subplot(1, 4, 4)
plt.imshow(cv2.cvtColor(resized_image, cv2.COLOR_BGR2RGB))
plt.title('Resized Image by Seam Carving')
plt.axis('off')

plt.show()

# Lưu ảnh kết quả
cv2.imwrite('image_with_seam.jpg', image_with_seam)
